...
function mapLive(chain) {
  ...
  let divergence = null;
  if (conflict) {
    const dmap = Object.fromEntries(decisions.map((d) => [d.id, d]));
    const a = dmap[conflict.from] || {};
    const b = dmap[conflict.to] || {};
    divergence = {
      field: a.field || b.field || "freight_mode",
      values: [a.value, b.value].filter((v) => v != null),
      who: [a.actor, b.actor].filter((v) => v != null),
      decisions: [conflict.from, conflict.to],
      cost: "$24,800",
    };
  }
